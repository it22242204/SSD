...
const URL = "http://localhost:8080/specialpayment"; // Update with your backend endpoint

const fetchSpecialPayments = async () => {
  return await axios.get(URL).then((res) => res.data);
};

function SpecialCollectionPaymentDetails() {
  ...
  useEffect(() => {
    fetchSpecialPayments().then((data) => setPayments(data.payments));
  }, []);

  const handleSearch = () => {
    fetchSpecialPayments().then((data) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Special Payment Details Report",
    onAfterPrint: () => alert("Special Payment Details Report Successfully Downloaded!"),
  });
      ...
      <div className="children_div_admin">
        ...
        <div className="tbl_con_admin" ref={ComponentsRef}>
          <h1 className="topic_inventory">
            Special Collection Payment <span className="sub_topic_inventory">Details</span>
          </h1>
              ...
              <tbody>
                {payments.map((payment, index) => (
                  <tr className="admin_tbl_tr" key={index}>
                    <td className="admin_tbl_td">{payment.contactname}</td>
                    ...
                    <td className="admin_tbl_td">
                      <button onClick={() => updateHandler(payment._id)} className="btn_dash_admin">
                        Update
                      </button>
                      <button onClick={() => deleteHandler(payment._id)} className="btn_dash_admin_dlt">
                        Delete
                      </button>
                    </td>
                  </tr>
...
export default SpecialCollectionPaymentDetails;